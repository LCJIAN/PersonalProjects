...
public class ReadPresenter extends RxPresenter<ReadContract.View>
    ...
    public void loadCategory(String bookId) {
        Disposable disposable = RestAPI.getInstance().readerService()
                .getBookChapters(Long.parseLong(bookId))
                .flatMap((Function<ResponseData<List<Chapter>>, Observable<Chapter>>) listResponseData -> Observable.fromIterable(listResponseData.data))
                .map(chapter -> {
                    BookChapterBean bean = new BookChapterBean();
                    bean.setBookId(bookId);
                    bean.setTitle(chapter.name);
                    bean.setLink(String.valueOf(chapter.index));
                    return bean;
                })
                .toList()
    ...
    public void loadChapter(String bookId, List<TxtChapter> bookChapters) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            // 网络中获取数据
            Single<ChapterInfoBean> chapterInfoSingle = RestAPI.getInstance().readerService()
                    .getBookChapterContent(Long.parseLong(bookChapter.getBookId()), Long.parseLong(bookChapter.getLink()))
                    .map(stringResponseData -> stringResponseData.data)
                    .map(s -> {
                        ChapterInfoBean bean = new ChapterInfoBean();
                        bean.setBody(s);
                        bean.setTitle(bookChapter.getTitle());
                        return bean;
                    })
                    .firstOrError();
