...
public class WTaskGattConfig extends AsyncTask<Void, Integer, Boolean> {
    ...
    private final List<byte[]> mUniCastAddresses;
    ...
    public WTaskGattConfig(Context context,
        ...
        this.mUniCastAddresses = uniCastAddresses;
    ...
    private boolean setUniCast() {
        ...
        list.add(new byte[]{(byte) mUniCastAddresses.size()});
        list.addAll(mUniCastAddresses);
        byte[] message = Utils.concatForByte(list);
        assert message != null;
