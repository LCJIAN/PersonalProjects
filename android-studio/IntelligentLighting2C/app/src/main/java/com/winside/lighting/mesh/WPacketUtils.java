...
public class WPacketUtils {
    ...
    public static Packet buildPacket(byte[] destAddress, byte[] message) {
        Packet packet = new Packet();

        byte[] data = new byte[6 + message.length];
        data[0] = "{".getBytes()[0];
        data[1] = (byte) (data.length - 4);
        System.arraycopy(destAddress, 0, data, 2, destAddress.length);
        System.arraycopy(message, 0, data, 4, message.length);
        data[data.length - 2] = calculateCRC(destAddress, message);
        data[data.length - 1] = "}".getBytes()[0];

        packet.setData(data);
        return packet;
    }

    private static byte calculateCRC(byte[] destAddress, byte[] message) {
        byte[] temp = new byte[destAddress.length + message.length];
        System.arraycopy(destAddress, 0, temp, 0, destAddress.length);
        System.arraycopy(message, 0, temp, destAddress.length, message.length);
