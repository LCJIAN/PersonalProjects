...
public class VoiceRecorderView extends AppCompatTextView implements View.OnLongClickListener, VoiceRecorder.AudioStateListener {

    //Y轴滑动的距离界限，超过此距离时，则判断为想要取消
    private static final int DISTANCE_Y_CANCEL = 50;
    //录音文件的存放位置
    private static final String AUDIO_DIR_PATH = Constants.DIRECTORY_VOICE.getPath();
    //音量等级的最大值
     ...
     * 则此时VoiceRecorder不再需要进行prepare了；若有触发LongClick时，则VoiceRecorder就需要release了
     */
    ...
    public VoiceRecorderView(Context context, AttributeSet attrs) {
        ...
        mVoiceRecorder = VoiceRecorder.getInstance(AUDIO_DIR_PATH);
        mVoiceRecorder.setOnAudioStateListener(this);
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case MSG_AUDIO_VOICE_UPDATE:
                        mDialogManager.updateVoiceLevel(mVoiceRecorder.getVoiceLevel(AUDIO_VOICE_MAX));
                        break;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                //若VoiceRecorder还没有进入prepare状态，则直接reset并返回即可
                if (!isStartPrepare) {
                    resetState();
                    return super.onTouchEvent(event);
                }
                //提示录音时间太短的dialog，有2种情况
                //1、还没开始录音时，VoiceRecorder已经进入prepare过程，但还没prepare完成，而用户又抬起按钮，此时需要提示dialog并release
                ...
                if (!isRecoreding || (mRecordTime < AUDIO_RECORDER_TIME_MIN && mCurrentState == STATE_RECORDING)) {
                    //提示时间太短的dialog，由于只有VoiceRecorder Prepare完成了才调用了showRecordingDialog()，所以此处需要判断是否需要调用show dialog
                    if (!isRecoreding) {
                        mDialogManager.showRecordingDialog();
                    }
                ...
                if (mCurrentState == STATE_RECORDING) {     //正常录音并完成
                    ...
                    if (mAudioRecorderFinishListener != null) {
                        mAudioRecorderFinishListener.onFinishRecorder(mRecordTime, mVoiceRecorder.getCurrentFilePath());
                    }
