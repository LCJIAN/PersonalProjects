...
public class Spans extends SpannableStringBuilder {
    ...
    public Spans(CharSequence text) {
        super(text);
    }

    public Spans(CharSequence text, Object... spans) {
        super(text);
    ...
    public Spans(CharSequence text, Object span) {
        super(text);
        setSpan(span, 0, text.length());
    }

    /**
     * Sets span objects to the text. This is more efficient than creating a new instance of Spans
     ...
     * @return this {@code Spans}.
     */
    public Spans append(CharSequence text, Object... spans) {
        append(text);
        for (Object span : spans) {
    ...
    public Spans append(CharSequence text, Object span) {
        append(text);
        setSpan(span, length() - text.length(), length());
        return this;
    }
     ...
     * @return {@code Spans}.
     */
    public Spans findAndSpan(CharSequence textToSpan, GetSpan getSpan) {
        int lastIndex = 0;
        while (lastIndex != -1) {
            lastIndex = toString().indexOf(textToSpan.toString(), lastIndex);
            if (lastIndex != -1) {
                for (Object span : getSpan.getSpan()) {
                    setSpan(span, lastIndex, lastIndex + textToSpan.length());
                }
